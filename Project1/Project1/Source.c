...
float sum_matches(unsigned char split_str[], int size)
{
	...
	for (i = 0; i < deltaSize; i++)
	{
		...
		{
			sum += count_repit * (count_repit - 1) / (float)(size * (deltaSize));//вынос инваринта
			count_repit = 0;
		}
	}
	return sum;
...
int key_size(unsigned char shifr_text[], int size, float key_long[])
{
	...
	float sum_index = 0.0;
	int number = size / 2 + 1;
	for (int i = 2; i < number; i++)//size/2 + 1 заменить на переменную
	{
	...
	for (int i = 2; i < number; i++)// тоже самое
	{
...
void get_key(unsigned char* text, unsigned int size, unsigned char* key, unsigned int ksize)
{
	...
	for (int i = 0; i < ksize; i++)
	{
		...
		for (unsigned int i = 0; i < ksize; i++)
		{
			...
			for (int j = 0; j < 256; j+= 8)//razveeeeeertka
			{
				if (keystat[i][j] > max)
				{
					max = keystat[i][j];
